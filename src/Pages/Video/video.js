...
// ✅ STEP 2: Check if format is browser-unsupported (needs server conversion)
...
const videos = [
  ...
  // ✅ EXAMPLE: How to add AVI / MKV / WMV / MOV / WebM / FLV videos in future
  // { id: 63, src: "http://yourserver.com/videos/sample.webm", ... },  ← works natively
  // { id: 64, src: "http://yourserver.com/videos/sample.mov",  ... },  ← Safari only
  // { id: 65, src: "http://yourserver.com/videos/sample.avi",  ... },  ← needs FFmpeg conversion
];
...
const Video = () => {
  ...
  // ✅ NEW: Reset error state when video changes
  ...
  // ✅ STEP 3: Get the format name for display in error message
    ...
    <div className="video">
      <div className="videoPostSection">
        ...
        >
          ...
          {/* ✅ STEP 4: Show error banner if format is unsupported */}
          {isUnsupportedFormat(video.src) && (
            ...
            >
              ...
              browsers. Please convert this video to MP4 or WebM using FFmpeg on
              your server.
            </div>
          )}

          {/* ✅ STEP 5: Show error if video failed to load */}
          ...
          >
            {/*
              ✅ Using <source> instead of src= directly on <video>
              This is the correct way to support multiple formats.
              Browser picks the first <source> it can play.
            */}
            <source src={video.src} type={getVideoType(video.src)} />
            {/*
              ✅ If you have a WebM version of the same video, add it as fallback:
