  ...
  const SeekBar = () => (
    <div style={{ position: "relative", height: 20, display: "flex", alignItems: "center", marginBottom: 2 }}>
      ...
      <div style={{
        ...
        width: `${progressPct}%`, background: "#e50914",
  ...
  const PlaylistPanel = () => (
    <div style={{
      ...
      ...(isMobile ? {
        ...
        borderTop: "1px solid #222", borderRadius: "18px 18px 0 0",
      } : {
        width: 290, borderLeft: "1px solid #1e1e1e", flexShrink: 0,
      }),
    }}>
      ...
      {/* header */}
      <div style={{
        padding: "10px 16px 12px", borderBottom: "1px solid #1e1e1e",
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button onClick={() => fileInputRef.current?.click()} style={{
            background: "#e50914", border: "none", borderRadius: 7,
          ...
          {playlist.length > 0 && (
            <button onClick={() => { setPlaylist([]); setCurrentIndex(0); setPlaying(false); }} style={{
              ...
              color: "#666", padding: "7px 11px", cursor: "pointer", fontSize: 12,
            }}>Clear</button>
          )}
          {isMobile && (
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {playlist.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: "#444", fontSize: 13 }}>
            ...
            <p style={{ margin: 0, fontSize: 12 }}>Tap "+ Add Files" to start</p>
          </div>
        ) : (
          playlist.map((item, i) => (
            <div key={item.id}
            ...
            >
              <div style={{
                width: 34, height: 34, borderRadius: 6, background: "#1a1a1a",
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{
                  color: i === currentIndex ? "#fff" : "#aaa", fontSize: 13, margin: 0,
      ...
      {!isMobile && (
        <div style={{ padding: "8px 14px", borderTop: "1px solid #181818", fontSize: 10, color: "#2e2e2e", lineHeight: 1.9, flexShrink: 0 }}>
      ...
      >
          ...
          {!currentMedia && (
            <div
              ...
              style={{
                ...
                color: "#555", gap: 14, cursor: "pointer",
            ...
            >
              <div style={{
                ...
                <svg width={isMobile ? 32 : 44} height={isMobile ? 32 : 44}
                  viewBox="0 0 24 24" fill="none"
                  stroke={dragOver ? "#e50914" : "#444"} strokeWidth="1.5">
                  <path d="M12 16V4M12 4L8 8M12 4l4 4M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2"
                    strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <p style={{ fontSize: isMobile ? 16 : 18, color: dragOver ? "#e50914" : "#555", margin: 0, fontWeight: 500 }}>
                {isMobile ? "Tap to open a video or audio file" : "Drop files here or click to browse"}
              </p>
              <p style={{ fontSize: 12, color: "#333", margin: 0 }}>MP4 · MKV · MOV · MP3 · WAV · FLAC</p>

              {isMobile && (
                <button style={{
                  marginTop: 8,
                  background: "#e50914", border: "none", borderRadius: 10,
          ...
          {currentMedia && isAudio && (
            <div style={{
              ...
              <div style={{
                ...
                borderRadius: "50%", background: "#0f0f0f",
                ...
                <svg width={isMobile ? 48 : 64} height={isMobile ? 48 : 64}
                  viewBox="0 0 24 24" fill="none" stroke="#e50914" strokeWidth="1.2">
                  <path d="M9 18V5l12-2v13" strokeLinecap="round" strokeLinejoin="round" />
                  <circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" />
                </svg>
              </div>
              <p style={{ color: "#fff", fontSize: isMobile ? 16 : 20, fontWeight: 600, margin: "0 0 6px", textAlign: "center", maxWidth: 300, padding: "0 16px" }}>
                {currentMedia.name}
              </p>
              <p style={{ color: "#555", fontSize: 12, margin: 0 }}>{formatSize(currentMedia.size)}</p>
              <div style={{ display: "flex", gap: 3, marginTop: 20, alignItems: "flex-end", height: 36 }}>
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 0 : 4 }}>
                ...
                <button onClick={togglePlay} style={{
                  ...
                  margin: "0 4px", touchAction: "manipulation",
                  WebkitTapHighlightColor: "transparent",
                }}>
                  {playing
                ...
                <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                  <Btn onClick={() => isMobile ? setShowVolumeSlider(v => !v) : setMuted(m => !m)} title="Volume">
                    {muted || volume === 0
                  ...
                  {isMobile && showVolumeSlider && (
                    <div style={{
                      ...
                      background: "#1a1a1a", borderRadius: 10, padding: "12px 10px",
                      display: "flex", flexDirection: "column", alignItems: "center",
                      gap: 6, zIndex: 60, boxShadow: "0 4px 24px rgba(0,0,0,0.8)",
                    }}>
                      <span style={{ color: "#888", fontSize: 10 }}>{Math.round((muted ? 0 : volume) * 100)}%</span>
                      <input type="range" min={0} max={1} step={0.05} value={muted ? 0 : volume}
                        onChange={(e) => { setVolume(parseFloat(e.target.value)); setMuted(false); }}
                        style={{ writingMode: "vertical-lr", direction: "rtl", height: 90, accentColor: "#e50914" }}
                      />
                    </div>
                  )}
                </div>
                ...
                {/* TIME desktop */}
                {!isMobile && (
                  <span style={{ color: "#ccc", fontSize: 12, whiteSpace: "nowrap", marginLeft: 4 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  {showSpeedMenu && (
                    <div style={{
                      ...
                      borderRadius: 8, overflow: "hidden", marginBottom: 6, zIndex: 70, minWidth: 110,
                    }} onClick={(e) => e.stopPropagation()}>
                      {speeds.map((s) => (
                ...
                {isVideo && !isMobile && (
                  <div style={{ display: "flex", alignItems: "center", gap: 5, marginLeft: 4 }}>
                    ...
                    <input type="range" min={50} max={150} value={brightness}
                      ...
                      style={{ width: 58, accentColor: "#e50914", cursor: "pointer" }}
                    />
                  </div>
                )}
                ...
                {isVideo && (
                  <Btn onClick={toggleFullscreen} title="Fullscreen">
                    {fullscreen
                      ? <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z" /></svg>
        ...
        {!isMobile && (
          <button onClick={() => setShowPlaylist(p => !p)} style={{
            position: "absolute", right: showPlaylist ? 298 : 8, bottom: 80,
            background: "#161616", border: "1px solid #2a2a2a",
        ...
        {isMobile && showPlaylist && (
          <>
            <div onClick={() => setShowPlaylist(false)} style={{
              position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 90,
            }} />
            <PlaylistPanel />
          </>
        )}
