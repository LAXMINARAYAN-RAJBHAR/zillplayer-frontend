...
export default function LocalMediaPlayer({ sideNavbar }) {
  ...
  // On desktop, default playlist open
  useEffect(() => {
    if (!isMobile) setShowPlaylist(true);
    else setShowPlaylist(false);
  }, [isMobile]);
  ...
  // ── SEEK BAR ──
  const SeekBar = () => (
    <div style={{ position: "relative", height: 18, display: "flex", alignItems: "center", marginBottom: 6 }}>
      <div style={{
        position: "absolute", left: 0, right: 0, height: 4,
        background: "#222", borderRadius: 2,
      }} />
      <div style={{
        position: "absolute", left: 0, height: 4,
        width: `${progressPct}%`, background: "#e50914", borderRadius: 2,
        pointerEvents: "none",
      }} />
      <input
        ...
        style={{
          ...
          height: 18, cursor: "pointer", margin: 0,
        }}
      />
    </div>
  );
  ...
  const PlayPauseBtn = ({ size = 44 }) => (
    <button onClick={togglePlay} style={{
      ...
      {playing
        ? <svg width={size * 0.45} height={size * 0.45} viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16" /><rect x="14" y="4" width="4" height="16" /></svg>
        : <svg width={size * 0.45} height={size * 0.45} viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
      }
    </button>
  );
  ...
  return (
    <div style={{
      ...
      height: isMobile ? "100dvh" : "calc(100vh - 60px)",
      ...
      <div
        ...
        style={{
          ...
          minHeight: isMobile ? "55vw" : undefined,
          maxHeight: isMobile ? "60vw" : undefined,
        }}
      ...
      >
        ...
        {!currentMedia && (
          <div style={{
            ...
            color: "#555", gap: 12, cursor: "pointer",
                    ...
                    entry.createReader().readEntries((entries) => {
                      entries.forEach((en) => {
                        if (en.isFile) en.file((f) => {
                          if (f.type.startsWith("video/") || f.type.startsWith("audio/")) files.push(f);
                        });
                      });
          ...
          >
            <div style={{
              ...
              <svg width={isMobile ? 28 : 40} height={isMobile ? 28 : 40} viewBox="0 0 24 24" fill="none"
                stroke={dragOver ? "#e50914" : "#555"} strokeWidth="1.5">
                <path d="M12 16V4M12 4L8 8M12 4l4 4M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2"
                  strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <p style={{ fontSize: isMobile ? 15 : 18, color: dragOver ? "#e50914" : "#555", margin: 0 }}>
              {isMobile ? "Tap to browse files" : "Drop video or audio files here"}
            </p>
            {!isMobile && <p style={{ fontSize: 13, color: "#444", margin: 0 }}>or click to browse</p>}
            <p style={{ fontSize: 11, color: "#333", margin: 0 }}>MP4, MKV, AVI, MOV, MP3, WAV, FLAC, AAC</p>
          </div>
        )}
            ...
            <div style={{
              ...
              {playing && (
                <div style={{
                  ...
                  background: "conic-gradient(from 0deg, transparent 0%, #e5091420 50%, transparent 100%)",
                }} />
              )}
              <svg width={isMobile ? 40 : 64} height={isMobile ? 40 : 64} viewBox="0 0 24 24"
                fill="none" stroke="#e50914" strokeWidth="1.2">
                <path d="M9 18V5l12-2v13" strokeLinecap="round" strokeLinejoin="round" />
                <circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" />
              </svg>
            </div>
            <p style={{ color: "#fff", fontSize: isMobile ? 15 : 20, fontWeight: 600, margin: "0 0 4px", textAlign: "center", maxWidth: 320 }}>
              {currentMedia.name}
            </p>
            <p style={{ color: "#666", fontSize: 12, margin: 0 }}>{formatSize(currentMedia.size)}</p>
            <div style={{ display: "flex", gap: 3, marginTop: 16, alignItems: "flex-end", height: 32 }}>
        ...
        {currentMedia && (
          <div
            style={{
              ...
              background: isVideo ? "linear-gradient(transparent, rgba(0,0,0,0.92))" : "transparent",
              padding: isMobile ? "24px 12px 10px" : "40px 16px 16px",
          ...
          >
            ...
            {isMobile && (
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                <span style={{ color: "#aaa", fontSize: 11 }}>{formatTime(currentTime)}</span>
                <span style={{ color: "#555", fontSize: 11 }}>{formatTime(duration)}</span>
              </div>
            )}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 2 : 6 }}>
              ...
              {/* REWIND — hide on small mobile */}
              {!isMobile && iconBtn(() => seek(-10),
              ...
              {/* PLAY/PAUSE */}
              <PlayPauseBtn size={isMobile ? 38 : 44} />

              {/* FORWARD — hide on small mobile */}
              {!isMobile && iconBtn(() => seek(10),
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              ...
              {/* VOLUME — tap icon on mobile to toggle slider */}
              <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                {iconBtn(
                  () => isMobile ? setShowVolumeSlider(v => !v) : setMuted((m) => !m),
                  muted || volume === 0
                ...
                {/* Volume slider — inline on desktop, popup on mobile */}
                {(!isMobile || showVolumeSlider) && (
                  <div style={{
                    ...(isMobile ? {
                      position: "absolute", bottom: "110%", left: "50%",
                      transform: "translateX(-50%)",
                      background: "#1a1a1a", borderRadius: 8, padding: "10px 8px",
                    ...
                    {isMobile ? (
                      <input type="range" min={0} max={1} step={0.05}
                        ...
                        style={{ writingMode: "vertical-lr", direction: "rtl", height: 80, accentColor: "#e50914", cursor: "pointer" }}
                      />
                    ) : (
              ...
              {/* TIME (desktop) */}
              {!isMobile && (
                <span style={{ color: "#ccc", fontSize: 13, marginLeft: 4, whiteSpace: "nowrap" }}>
              ...
              <div style={{ position: "relative" }}>
                <button onClick={(e) => { e.stopPropagation(); setShowSpeedMenu((s) => !s); }} style={{
                  background: "none", border: "1px solid #444", borderRadius: 4,
                  color: speed !== 1 ? "#e50914" : "#ccc",
                  cursor: "pointer", padding: "3px 6px",
                  fontSize: isMobile ? 11 : 12, fontWeight: 600, touchAction: "manipulation",
                }}>
                  {speed}x
                </button>
                {showSpeedMenu && (
                  <div style={{
                    ...
                    borderRadius: 6, overflow: "hidden", marginBottom: 6, zIndex: 30,
                  }} onClick={(e) => e.stopPropagation()}>
                    {speeds.map((s) => (
                      <div key={s} onClick={() => { setSpeed(s); setShowSpeedMenu(false); }} style={{
              ...
              {/* FULLSCREEN */}
              {isVideo && iconBtn(toggleFullscreen,
                fullscreen
              ...
              {/* PLAYLIST TOGGLE (mobile — inside controls) */}
              {isMobile && iconBtn(
                () => setShowPlaylist((p) => !p),
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
      ...
      {/* ── PLAYLIST PANEL ── */}
      {/* Mobile: slide-up sheet | Desktop: sidebar */}
      {showPlaylist && (
        <div style={{
          ...(isMobile ? {
            position: "fixed", bottom: 0, left: 0, right: 0,
            height: "50dvh", zIndex: 50,
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button onClick={() => fileInputRef.current?.click()} style={{
                ...
                cursor: "pointer", fontSize: 12, fontWeight: 600, touchAction: "manipulation",
              }}>
                + Add Files
              </button>
              {playlist.length > 0 && (
                <button onClick={() => { setPlaylist([]); setCurrentIndex(0); setPlaying(false); }} style={{
          ...
          >
            {playlist.length === 0 ? (
              <div style={{ padding: 32, textAlign: "center", color: "#444", fontSize: 13 }}>
                ...
                <p style={{ fontSize: 12 }}>Tap "Add Files" to get started</p>
              </div>
            ) : (
              playlist.map((item, i) => (
                <div key={item.id}
                  onClick={() => {
                    setCurrentIndex(i);
                    setShowPlaylist(isMobile ? false : true);
                    if (mediaRef.current) {
                ...
                >
                  <div style={{
                    ...
                    {i === currentIndex && playing ? (
                      <div style={{ display: "flex", gap: 2, alignItems: "flex-end", height: 16 }}>
                        {[1, 2, 3].map((j) => (
                          <div key={j} style={{
                            width: 3, background: "#e50914", borderRadius: 1,
                            height: `${8 + j * 3}px`,
                  ...
                  <button onClick={(e) => removeFromPlaylist(item.id, e)} style={{
      ...
      {/* Desktop playlist toggle button */}
      {!isMobile && (
      ...
      {/* Mobile backdrop for playlist */}
      {isMobile && showPlaylist && (
        <div onClick={() => setShowPlaylist(false)} style={{
      ...
      <style>{`
        @keyframes eq { from { transform: scaleY(0.5); } to { transform: scaleY(1.5); } }
